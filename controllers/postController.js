...
const createPost = asyncHandler(async (req, res) => {
  ...
  const { content, tweetTime } = req.body;
  if (!content) {
...
const handlePostLike = asyncHandler(async (req, res) => {
  ...
  const checkedPost = await Post.findOne({ likedBy: req.user._id, _id:postId});
  console.log(checkedPost);
  if(checkedPost){
    const removeLike= await Post.findOneAndUpdate({_id:postId, likedBy: req.user._id}, {$pull:{ likedBy: req.user._id}}, {new: true}) 
    console.log(removeLike)
    if(removeLike){
      res.status(200).json({message:'Unliked successfully!', data: removeLike})
    }
  }else{
